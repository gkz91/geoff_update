...
%% Inputs
dataname = 'detection_data_mock_032421.mat'; %rename me
mgene_data = load('processed_delRn_detection_data_std_032421.mat'); %rename me
...
FAM_auto = 'N'; %auto filter = Y, manual filter = N
CV_auto = 'Y'; %auto CV filter = Y, manual filter = N
delX = 10; %delta X for drop# filter (10)
nbins_Otsu = 50; %for Otsu thresholding (50)
X_Otsu = [0.5 0.5 1 1 1 1]; %change Otsu threshold at cycle i (counter bias from noisy drops, 0.5 at cycle 1)
T_CV = [1, 0.3, 0.3, 0.3, 0.3, 0.3]; %manual CV threshold for automatic filtering
T_FAM = [1 2e4; 3e3 15e3; 1 0.5e4; 1 0.5e4; 1 0.5e4; 1 0.5e4]; %manual FAM threshold
...
baseline_adjust = [1 1]; %adjust ROX between cycles
...
T_delRn_FAM = [0.025 T_C40; 0.025 T_C40; 0.025 T_C40; 0.025 T_C40; 0.025 T_C40; 0.025 T_C40]; %manual delRn threshold
%T_delRn = [-2 10; -2 10; -2 10; -2 10; -2 10; -2 10; -2 10; -2 10]; %no filter
edges_delRn_FAM = linspace(-0.2,2,nbins);



%delRn (CY5) thresholds (final cleanup)
T_delRn_CY5 = [-0.25 0.25; 0 0.3; -0.25 0.5; -0.25 1; -0.25 1; -0.25 1.5; -0.25 1.5; -0.25 2]; %manual delRn threshold
%T_delRn = [-2 10; -2 10; -2 10; -2 10; -2 10; -2 10; -2 10; -2 10]; %no filter
...
for i = 1 : C
    ...
    %delRn (Cy5)
    delRn_CY5{i} = (CY5_temp_5./ROX_temp_5) - Rn_CY5_avg(1)*baseline_adjust(i);
