...
%3-7-21 v7.2 add Ostsu threshold for CV data
...
delX = 10; %delta X for drop# filter
nbins_Otsu = 50; %for Otsu thresholding
X_C1 = 0.5; %decrease Otsu threshold at cycle (counter bias from noisy drops)
...
%         end
    ...
    %adjust cycle 1 threshold
    if i == 1
        
        T2_CV_Otsu(1) = T2_CV_Otsu(1)*X_C1;
        
    end
    
    %save section above Otsu threshold
    for j = 1 : N_delx
        
        if CV_section(i,j) >= T2_CV_Otsu(i)
...
for i = 1 : C
    ...
    yline(T2_CV_Otsu(i),'--r','linewidth',1);
    title(['Cycle ',num2str(cycle(i),'%2.0f')])
    ylabel('FAM CV (a.u.)')
    axis([0 inf 0 inf])
