%Process PCR detection data for M gene control experiments
%4-5-21 v6.2 remove Cy5 filtering and gain correction
%Geoff Zath
...
%% Inputs
dataname = 'detection_data_std_041621_outlier.mat'; %rename me
...
%edges_CY5 = linspace(0,2,nbins);

%time inside thresholds
T_time = [1 1.75; 1 1.75; 1 1.75; 1 1.75; 1 1.75; 1 1.75; 1 1.75; 1 1.75; 1 1.75];
%T_time = [0 10; 0 10; 0 10; 0 10; 0 10; 0 10; 0 10; 0 10]; %no filter
edges_time = linspace(0,4,nbins);

%ROX thresholds

T_ROX = [0.05 0.2; 0.05 0.2; 0.05 0.2; 0.05 0.2; 0.05 0.2; 0.05 0.2; 0.05 0.2; 0.05 0.2; 0.05 0.2];
%T_ROX = [0 10; 0 10; 0 10; 0 10; 0 10; 0 10; 0 10; 0 10]; %no filter
edges_ROX = linspace(0,0.5,nbins);

%delRn (FAM) thresholds (final cleanup)
T_delRn_FAM = [-0.2 0.5; -0.2 0.5; -0.2 0.5; 0 1; 0.5 2; 1 3; 1.5 4; 2 4; 2 4]; 
%T_delRn = [-2 10; -2 10; -2 10; -2 10; -2 10; -2 10; -2 10; -2 10]; %no filter
edges_delRn_FAM = linspace(-0.2,5,nbins);

% %delRn (CY5) thresholds (final cleanup)
% T_delRn_CY5 = [-0.2 0.2; -0.2 0.5; 0 0.8; 0.25 1; 0.5 1.5; 0.5 1.5; 0.5 1.5; 0.5 1.5]; 
% %T_delRn = [-2 10; -2 10; -2 10; -2 10; -2 10; -2 10; -2 10; -2 10]; %no filter
% edges_delRn_CY5 = linspace(-0.2,3,nbins);
...
%ratio_gain_ROX(6) = 0.7; %adjust for high ROX signal
...
%     
%     delRn_temp_2 = delRn_temp_1(delRn_temp_1 > T_delRn(i,1));
%     delRn_temp_3 = delRn_temp_2(delRn_temp_2 < T_delRn(i,2));
%     
...
for i = 1 : C
    ...
    delRn_FAM_CV_FINAL(i) = delRn_FAM_std_FINAL(i)/delRn_FAM_avg_FINAL(i);
    delRn_FAM_SE_FINAL(i) = std(delRn_FAM_FINAL{i})/sqrt(length(delRn_FAM_FINAL{i}));
...
%     'ROX','TIME','FAM_corr','ROX_corr','delRn_FAM_SE_FINAL')
...
%subplot of detection delRn (FAM_ histograms)
figure(1); clf(1)
