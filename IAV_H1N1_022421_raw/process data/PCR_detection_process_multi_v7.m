%Process PCR detection data for MOI 0.1 (H1N1) burst size IAV drops
%3-1-21 v7 add filter for drop measurements over time
...
%% Inputs
dataname = 'detection_data_IAV_022421.mat'; %rename me
...
C40_mgene_avg = 0.4314; %from std curve
C40_mgene_std = 0.0256;
T_C40 = C40_mgene_avg - C40_mgene_std;

edges_CY5 = linspace(0.4,2,nbins);

%FAM thresholds by drop #
T_FAM = [1 1.5e4; 1 1.25e4; 1.15e3 1.05e4; 1 7e3; 1 2.1e3; 1 1.25e4];
edges_FAM = linspace(0,0.3,nbins);

%time inside thresholds
T_time = [4.5 6.5; 4.5 6.5; 4.5 6.5; 4.5 6.5; 4.5 6.5; 4.5 6.5];
%T_time = [0 10; 0 10; 0 10; 0 10; 0 10; 0 10; 0 10; 0 10]; %no filter
edges_time = linspace(0,10,nbins);

%ROX thresholds
T_ROX = [0.5 0.8; 0.5 0.8; 0.5 0.8; 0.5 0.8; 0.5 0.8; 0.5 0.8; 0.5 0.8; 0.5 0.8];
%T_ROX = [0 10; 0 10; 0 10; 0 10; 0 10; 0 10; 0 10; 0 10]; %no filter
edges_ROX = linspace(0,1,nbins);

%delRn (FAM) thresholds (final cleanup) (set min at <+1% at cycle 1, set max at std cycle 40 mean - std)
T_delRn_FAM = [0.025 0.406; 0.025 0.406; 0.025 0.406; 0.025 0.406; 0.025 0.406; 0.025 0.6]; 
...
delRn_FAM_posCI_C1 = delRn_FAM_avg(1) + Z*delRn_FAM_std(1)/sqrt(length(delRn_FAM{1}));
...
for i = 1 : C
    ...
    yline(T_time(i,1),'-k','linewidth',1);
    yline(T_time(i,2),'-k','linewidth',1);
...
for i = 1 : C
    ...
    yline(T_ROX(i,1),'-k','linewidth',1);
    yline(T_ROX(i,2),'-k','linewidth',1);
    xline(T_FAM(i,1),'-k','linewidth',1);
    xline(T_FAM(i,2),'-k','linewidth',1);
    title(['Cycle ',num2str(cycle(i),'%2.0f')])
