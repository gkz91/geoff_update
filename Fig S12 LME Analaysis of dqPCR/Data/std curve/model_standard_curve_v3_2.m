%qPCR model standard curve based on efficiency model fit to 7-3-20 drop detection
%data and 7-3-20 dilution series
%2-28-21 v3.2 remove PCR efficiency curve + update graphs for burst size
%paper
%Geoff Zath

%delRn normalization
%10^3 cpd in 50 um drops (10^7 copies/uL)
...
N_span = 1000; %number of model curves (increases resolution of span Ct fit)
E_PCR = 0.8763; %PCR efficiency fraction (7-3-20 data) (0.95 high eff)
conc_fit = 171; %100 cpd fit post split (171 cpd) (5-13-20 data)
Rn_range = [0.5 2]; %range of linear region
cycle_graph = 20; %single cycle for figure 3
split_ratio = 1; %split ratio of EVO chip


%efficiency fitting
N_fit = 40; %number of data points measured for fit
F_0 = 1; %initial fluorescence at cycle 1 (arbitrary, can't be zero)
a_eff_range = [0.00 0.03]; %initial guess of min efficiency
b_eff_range = [2 3]; %intitial guess of slope
c_eff_range = [16 17]; %initial guess of C_0.5 range
d_eff_range = [0.95 1]; %initial guess of max efficiency
...
cycle_std = 20; %cycle data to convert
...
%fitting data (~100 cpd)
filename = 'processed_delRn_detection_data_std_070320_fixedfilter.mat';
PCR_curve = load(filename);
D_avg_curve = PCR_curve.delRn_avg_FINAL;
D_std_curve = PCR_curve.delRn_std_FINAL;
...
%                 for h = cycle_thresh : N_cycles
%                     
%                     idx = 1 + h - cycle_thresh;
%                     
%                     model_thresh(idx) = model_iter(h);
%                     
%                 end
...
%convert C_1/2 values to concentration

slope_stdc = -1/log10(E_PCR + 1);
...
%save data
stats_savename = ['eff_stdcurve' filename(10:end)];
...
%plot(cycle40,model_best,'-','color',gray(128,:),'linewidth',1)
errorbar(cycle_curve,D_avg_curve,D_std_curve,'.','color',gray(128,:),'linewidth',0.5,...
    'markersize',7)
...
% 
% legend('SCFPE PCR efficiency',['SCFPE \DeltaR_N (R^2 = ',num2str(R_sq_eff_best,'%1.4f'),')'],...
%     [num2str(conc_fit,'%1.2E'),' cpd'],...
%     'fontsize',6,'location','nw')

legend(['SCF-E fit (R^2 = ',num2str(R_sq_eff_best,'%1.4f'),')'],...
    [num2str(conc_fit,'%1.2E'),' cpd'],...
    'fontsize',6,'location','nw')
% legend(['SCFPE (R^2 = ',num2str(R_sq_eff_best,'%1.4f'),')'],...
%     [num2str(conc_fit,'%1.2E'),' cpd'],...
%     'fontsize',6,'location','nw')
...
for i = 1 : N_span
    ...
    plot(cycle40,model_scaled(1:40,i),'-','linewidth',3,'color',color_span(i,:))
...
set(gca,'fontsize',8,'linewidth',0.5,'yscale','log','xminortick','on','yminortick','on','layer','top','ticklength',[0.015 1])
%axis([0 41 -0.2 2.5])
axis([0 41 1e-2 5])
set(gcf, 'Position',  [100, 100, 405,230])
...
semilogy(model_scaled(cycle_graph,:),conc_stdc,':k','linewidth',1.5)




%plot min and max of valid region at cycle 20
x_min = D_avg_curve(1) + 3*D_std_curve(1); %copies/uL
x_max = D_avg_curve(end) - 3*D_std_curve(end); %copies/uL
...
xlabel('\DeltaRn (a.u.)')
%ylabel('C_{RNA} (cpd)')
ylabel('C_{RNA,virtual} (copies/uL)')
%title(['Standard Curve (Cycle ',num2str(cycle_graph,'%2.0f'),')'])
%set(gca,'fontsize',14,'linewidth',1)
%axis([0 2 1e-2 1e6])
axis([1e-2 5 1e-1 1e6])

set(gca,'fontsize',8,'linewidth',0.5,'yscale','log','xscale','log','xminortick','on','yminortick','on','layer','top','ticklength',[0.015 1])

%set(gcf, 'Position',  [100, 100, 398,220])
set(gcf, 'Position',  [100, 100, 405,230])
